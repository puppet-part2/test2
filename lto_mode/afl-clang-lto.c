...
static void edit_params(u32 argc, char** argv) {
  ...
  /* There are two ways to compile afl-clang-lto. In the traditional mode, we
  ...
  cc_params[cc_par_cnt++] = "-flto=full";
...
int main(int argc, char** argv) {
...
#ifdef USE_TRACE_PC
    SAYF(cCYA "afl-clang-lto [tpcg] " cBRI VERSION  cRST " by <lszekeres@google.com>\n");
#else
    SAYF(cCYA "afl-clang-lto " cBRI VERSION  cRST " by <lszekeres@google.com>\n");
#endif /* ^USE_TRACE_PC */
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "  CC=%s/afl-clang-lto ./configure\n"
         "  CXX=%s/afl-clang-lto++ ./configure\n\n"
